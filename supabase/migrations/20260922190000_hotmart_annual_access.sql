-- Product 8575181 grants one calendar year of access from payment approval.
-- NULL keeps the existing, non-expiring access for manual and older grants.
alter table public.clients add column if not exists access_expires_at timestamptz;
alter table public.allowed_emails add column if not exists access_expires_at timestamptz;
alter table public.allowed_emails add column if not exists access_product_id bigint;
alter table public.purchases add column if not exists product_id bigint;
alter table public.purchases add column if not exists last_event_at timestamptz;
alter table public.purchases add column if not exists access_expires_at timestamptz;

comment on column public.clients.access_expires_at is
  'Latest expiry of a valid purchase of Hotmart product 8575181.';
comment on column public.allowed_emails.access_expires_at is
  'When set, access is denied at or after this instant.';

create index if not exists purchases_annual_access_idx
  on public.purchases (client_id, access_expires_at desc)
  where product_id = 8575181 and access_expires_at is not null;
...
as $$
  select not exists (
    ...
    where email = lower(trim(email_to_check))
      and source = 'manual' and active = false
  ) and (
    exists (
      select 1 from public.allowed_emails
      where email = lower(trim(email_to_check))
    ...
    or exists (
      ...
      where c.email = lower(trim(email_to_check))
        and p.product_id = 8575181
        and p.access_expires_at > now()
    )
  );
$$;

-- Existing browser sessions must also lose direct data access after expiry.
alter policy journals_select_own on public.journals
  using (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));
alter policy journals_insert_own on public.journals
  with check (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));
alter policy journals_update_own on public.journals
  using (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'))
  with check (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));

alter policy aurora_conversations_owner_select on public.aurora_conversations
  using (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));
alter policy aurora_conversations_owner_insert on public.aurora_conversations
  with check (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));
alter policy aurora_conversations_owner_update on public.aurora_conversations
  using (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'))
  with check (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));
alter policy aurora_conversations_owner_delete on public.aurora_conversations
  using (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));

alter policy aurora_messages_owner_select on public.aurora_messages
  using (
    public.is_email_allowed(auth.jwt() ->> 'email') and exists (
      select 1 from public.aurora_conversations c
      where c.id = conversation_id and c.user_id = auth.uid()
    )
  );
alter policy aurora_messages_owner_insert on public.aurora_messages
  with check (
    public.is_email_allowed(auth.jwt() ->> 'email') and exists (
      select 1 from public.aurora_conversations c
      where c.id = conversation_id and c.user_id = auth.uid()
...
alter policy aurora_messages_owner_update on public.aurora_messages
  using (
    public.is_email_allowed(auth.jwt() ->> 'email') and exists (
      select 1 from public.aurora_conversations c
      where c.id = conversation_id and c.user_id = auth.uid()
...
alter policy aurora_messages_owner_delete on public.aurora_messages
  using (
    public.is_email_allowed(auth.jwt() ->> 'email') and exists (
      select 1 from public.aurora_conversations c
      where c.id = conversation_id and c.user_id = auth.uid()
...
alter policy aurora_usage_owner_select on public.aurora_usage
  using (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));
alter policy aurora_usage_owner_insert on public.aurora_usage
  with check (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));
alter policy aurora_usage_owner_update on public.aurora_usage
  using (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'))
  with check (auth.uid() = user_id and public.is_email_allowed(auth.jwt() ->> 'email'));
